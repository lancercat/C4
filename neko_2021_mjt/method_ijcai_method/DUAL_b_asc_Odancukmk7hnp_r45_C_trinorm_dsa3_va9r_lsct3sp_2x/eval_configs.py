...
from neko_2021_mjt.dss_presets.dual_no_lsct_32 import get_bench_dss,get_eval_dss;
def dan_mjst_eval_cfg(
        save_root,dsroot,
        log_path,iterkey,maxT=30,batch_size=1):

    if(log_path):
        epath=os.path.join(log_path, "closeset_benchmarks");
    else:
        epath=None;
    te_meta_path_chsjap, te_meta_path_mjst, mjst_eval_ds, chs_eval_ds=get_bench_dss(dsroot,25,30,batch_size);
    task_dict = {}
    ...
    # task_dict = arm_base_task_default2(task_dict, "base_mjst_", osdanmk7_eval_routine_cfg, 30,
...
def dan_mjst_eval_cfg_mb(
        save_root,dsroot,
        log_path,iterkey,maxT=30,batch_size=384):

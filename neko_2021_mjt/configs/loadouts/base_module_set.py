...
def arm_base_routine2(srcdst, prefix, routine_type,maxT, log_path, log_each,dsprefix):
    ...
    srcdst[prefix+"mjst"]["stream"]=prefix;
    return srcdst;
def arm_base_routine2m(srcdst, prefix, routine_type,maxT, log_path, log_each,dsprefix):
    srcdst[prefix+"mjst"]= routine_type(
        prototyper_name=prefix+"prototyper",
        sampler_name=prefix+"Latin_62_sampler",
        feature_extractor_name=prefix+"feature_extractor_cco",
...
def arm_base_eval_routine2m(srcdst,tname,prefix,routine_type,log_path,maxT):
    return routine_type(
        prototyper_name=prefix + "prototyper",
        sampler_name=prefix + "Latin_62_sampler",
        feature_extractor_name=prefix + "feature_extractor_cco",
...
def arm_base_task_default2(srcdst,prefix,routine_type,maxT,te_meta_path,datasets,log_path,name="close_set_benchmarks",measure_rej=False):
    ...
    return srcdst
def arm_base_task_default2m(srcdst,prefix,routine_type,maxT,te_meta_path,datasets,log_path,name="close_set_benchmarks"):
    te_routine={};
    te_routine=arm_base_eval_routine2m(te_routine,"close_set_benchmark",prefix,routine_type,log_path,maxT)
    srcdst[prefix+name]={
        "type": neko_odan_eval_tasks,
        "protoname": prefix+"prototyper",
        "temeta":
